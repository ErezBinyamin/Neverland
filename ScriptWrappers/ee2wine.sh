...
ee2wine() {   
  ...
  if [[ -z "${IMAGE_NAME:-}" ]]
  then
    docker_image_ls="$(docker image ls 2>/dev/null)"
    for name in '^ee2wine:installed' '^ee2wine:latest'
    do
	    if echo "${docker_image_ls}" | grep -q "${name}"
	    then
		    IMAGE_NAME="${name}"
	    fi
    done
    [[ -z "${IMAGE_NAME:-}" ]] && IMAGE_NAME='erezbinyamin/ee2wine:latest'
  fi
  ...
  read -p "Startup zerotier for virtual LAN gaming? (Y/N): " confirm
  printf "\n\n> ${confirm^^}\n\n"
